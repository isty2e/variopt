"""Private immutable state for the clearing genetic algorithm."""
...
class ClearingGAPopulationMember(FrozenGenericSlotsCompat, Generic[CandidateT]):
    """Evaluated candidate stored in clearing-GA population state.
    ...
    score : float
        Normalized score used for clearing-based selection pressure.
    """
...
@dataclass(frozen=True, slots=True)
class ClearingGAOptimizerState(FrozenGenericSlotsCompat, Generic[CandidateT]):
    """Explicit immutable optimizer state for the clearing GA.
    ...
    population : tuple[ClearingGAPopulationMember[CandidateT], ...], default=()
    ...
    buffered_members : tuple[ClearingGAPopulationMember[CandidateT], ...], default=()
        Newly evaluated members buffered until the next clearing update.
    """
    ...
    buffered_members: tuple[ClearingGAPopulationMember[CandidateT], ...] = ()
